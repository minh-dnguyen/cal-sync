...
export function GoogleColorModal({ open, onClose, onConfirm }: Props) {
  return (
    <Modal open={open} onClose={onClose} title="Google Calendar event colors" size="sm">
      <div className="space-y-4">
        <p className="text-sm text-gray-600 dark:text-gray-300">
          Google Calendar events can have individual colors. Would you like to keep them?
        </p>
        ...
        <div className="grid grid-cols-2 gap-3 pt-1">
          ...
          >
            ...
            <span className="text-[10px] text-gray-500 dark:text-gray-400 text-center leading-tight">
              Each event uses its original color
            </span>
          </button>
          ...
          >
            <div className="flex gap-1">
              {[0, 1, 2, 3].map((i) => (
                <span
                  key={i}
                  className="w-3 h-3 rounded-full bg-[#4285F4]"
                />
              ))}
            </div>
