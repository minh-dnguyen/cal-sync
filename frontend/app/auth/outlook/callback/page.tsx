...
function CallbackHandler() {
  ...
  useEffect(() => {
    ...
    api
      ...
      .catch((err) => {
        const detail = err?.response?.data?.detail ?? "Failed to connect Outlook Calendar.";
        setErrorMsg(detail);
        setStatus("error");
      });
  }, [params, router, qc]);
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-gray-950">
      <div className="flex flex-col items-center gap-3">
        ...
        <p className="text-sm text-gray-500 dark:text-gray-400">Connecting Outlook Calendar…</p>
      </div>
    </div>
  );
}
